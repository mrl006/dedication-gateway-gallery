...
const normalizeText = (text: string): string => {
  return text.toLowerCase().replace(/ğ/g, 'g').replace(/ü/g, 'u').replace(/ş/g, 's').replace(/ı/g, 'i').replace(/ö/g, 'o').replace(/ç/g, 'c').replace(/Ğ/g, 'g').replace(/Ü/g, 'u').replace(/Ş/g, 's').replace(/İ/g, 'i').replace(/Ö/g, 'o').replace(/Ç/g, 'c');
};
const CertificatesGrid = ({
  ...
  const filteredCertificates = useMemo(() => {
    ...
    return sortedCertificates.filter(certificate => {
      ...
      return normalizedName.includes(normalizedSearchTerm) || normalizedTitle.includes(normalizedSearchTerm);
    });
  ...
  return <>
      ...
      <div className="text-center mb-6">
        <p className="text-white/70">
          ...
          {visibleCertificates.length < filteredCertificates.length && <span className="ml-2 text-purple-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
        {visibleCertificates.map((certificate, index) => <CertificateCard key={certificate.id} certificate={certificate} priority={index < 6} // Prioritize first 6 images
      />)}
      </div>

      {/* Load More Button with intersection observer trigger */}
      {visibleCertificates.length < filteredCertificates.length && <div className="text-center mt-8">
          <button id="load-more-trigger" onClick={handleLoadMore} className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-6 py-2 md:px-8 md:py-3 rounded-lg transition-all duration-300 hover:shadow-lg text-sm md:text-base">
            Load More Certificates
          </button>
        </div>}

      {/* No results message */}
      {filteredCertificates.length === 0 && searchTerm && <div className="text-center mt-12">
